...
Pre-commit security gates: gitleaks (secrets in staged content) and semgrep (`--config=auto` on staged files) — either can block the commit. For semgrep false positives, drop `.semgrep-exclude-rules` at the repo root (one rule-id per line, `#` lines are comments) — the hook passes each line as `--exclude-rule=<id>`. Example: `~/.claude/git-hooks/.semgrep-exclude-rules.example`.
