...
## Subagent delegation
Route read-only subagent work to the `kimi` skill instead of Claude Explore/general-purpose agents — Kimi is cheaper and its 256k context keeps yours clean. Covers: codebase research, exploration, "how does X work", read-only audits (security/perf/dead-code), diff peer-review, mapping/summarizing a code region.
