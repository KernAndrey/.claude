...
7. Write `## Testing Strategy` in business/behavior terms — one entry per AC covering level, the rule, the refusals the spec states, the cases that form the rule's table, fixtures, idempotency, and mock boundaries, then the two closing coverage lines that map every `## Edge Cases & Risks` row and every `## Examples` entry to the case that covers it. Follow the template's worked example. A test pins a rule the spec states, not a line or a branch: list only the cases `## Behavior` distinguishes, and mark a race or other non-behavior row `not tested — <category>`. An AC that states no refusal carries the explicit note `no refusal stated`; silence there reads as an oversight and comes back as a finding. Architect adds file-level detail later.
