...
7. Write `## Testing Strategy` in business/behavior terms — one entry per AC covering level, success case, failure cases, boundary and variant cases, fixtures, idempotency, and mock boundaries, then the two closing coverage lines that map every `## Edge Cases & Risks` row and every `## Examples` entry to the case that covers it. Follow the template's worked example. An AC with no failure case carries the explicit note `no failure mode — <why>`; silence there reads as an oversight and comes back as a finding. Architect adds file-level detail later.
