...
2. **For EACH queue item**, find the implementing code AND the implementing test. Read the code; don't trust file names. Flag:
   ...
   - Code exists but no test pins its behavior → MUST FIX (missing coverage). An item the spec marks `not tested — <category>` (a race, or another non-behavior category) is not a gap.
