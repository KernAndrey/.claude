...
- **Lens B — AC → test target traceability (architecture side).** For each AC, verify the plan says where its test lands: a test file path and a level the project's harness supports. Cross-check against the AC → Implementation map — a row listing a code element but no test target is a gap. Which cases that test must contain (rule, stated refusals, case table) is Spec-Critic-Testing's ground; you check that the architecture leaves a home for the test and that the named file fits the project's test layout.
