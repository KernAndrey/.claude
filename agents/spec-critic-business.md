...
- **Lens L — Testing Strategy presence.** Verify `## Testing Strategy` is populated with real entries rather than the template placeholder, and that it is written in business/behavior terms like the rest of the Analyst's sections. Missing or placeholder-only = MAJOR, `route: analyst`. The coverage audit itself — per-AC rule, stated refusals, and case tables, edge-case and example traceability — belongs to Spec-Critic-Testing, which runs in the same batch. Report presence and language here; leave coverage depth to it.
...
- **Lens O — Assumptions validity.** Every item in `## Assumptions` must be a falsifiable statement (not a truism like "the system works correctly"). Flag assumptions that contradict observable code behavior (grep/read to verify when possible). Flag implicit assumptions discovered during your review that are NOT listed (e.g., "this flow assumes no concurrent modifications" but Assumptions doesn't mention concurrency). The Analyst contract requires four baseline categories: external service availability, data integrity preconditions, concurrency assumptions, and idempotency properties. Missing any of these four = MAJOR. Missing Assumptions section entirely = CRITICAL.
...
- **Lens Q — Sentinel check.** Verify exactly one `[SENTINEL]` marker exists in the Behavior section. The marker MUST be in the middle of the section (not in the first or last paragraph) — the purpose is to verify implementing agents read the full section, so a sentinel at the top or bottom defeats the canary. The marked item must be a specific, verifiable detail (not a vague requirement). If missing → CRITICAL. If placed at the start or end → MAJOR. If more than one → MINOR (recommend keeping the strongest, removing duplicates).
