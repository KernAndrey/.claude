...
- **Tests in the same pass as code.** Every rule or observable behavior the change adds or modifies gets a test that pins it — one rule across fields, write paths or roles is one table-driven test, not a test per branch. Follow "Tests that pass review" in `~/.claude/CLAUDE.md`. Run tests and paste passing output into the session before committing.
...
<critical>
- The pre-commit AI reviewer BLOCKS commits that add or change a rule no test pins. Write tests in the same pass as the code.
