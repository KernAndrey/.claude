...
Every fix in the ledger arrived without a test. The commit hook gates on **the configured coverage threshold for added production lines (95% by default)**, and its tests lens blocks a rule no test pins, so this step is what makes the commit possible at all — not an optional quality pass.
