...
- **Testing Strategy** — business/behavior terms only, one entry per AC: level (unit / integration / e2e), the rule, and the refusals the spec states — or `no refusal stated` when an AC states none. Fixture strategy in domain terms. Idempotency requirements. Mock boundaries described as "where real domain logic begins" — not as file paths. The case table, plus the `Edge Cases covered:` / `Examples covered:` mapping lines, belong to the full `/spec` run where the testing critic audits them; a preliminary spec carries the rule and the stated refusals per AC and no more.
