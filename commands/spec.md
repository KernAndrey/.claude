...
4. Verify `## Testing Strategy` has one entry per AC, each carrying a `Rule:` line and either a `Refusal:` line or an explicit `no refusal stated` note, and that it closes with the `Edge Cases covered:` and `Examples covered:` lines. An AC accounted for by an `Uncovered:` line passes this gate — the fix rounds ran and recorded what they could not close. An AC with neither an entry nor an `Uncovered:` line means the section was never audited: send it to the Analyst rather than shipping the spec with a silent hole.
