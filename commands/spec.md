...
**Important:** opencode in `--pure` mode cannot read files outside the project directory (`~/.claude/agents/` is auto-rejected). Use the symlink `.claude/agents-global/spec-critic-arch.md` instead. If the symlink doesn't exist, create it: `mkdir -p .claude/agents-global && ln -sf ~/.claude/agents/spec-critic-arch.md .claude/agents-global/spec-critic-arch.md` (see `~/.claude/guides/opencode-review-runner.md` for full setup).
