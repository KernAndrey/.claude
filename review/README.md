...
5. **Run review** —
   ...
   - Fan-out: three parallel lens calls (`bugs`, `architecture`,
     `tests`) reading `prompts/<lens>.md` + `prompts/common.md` + optional
     `.claude/review_prompt.md`. The chunked path appends the same file to
     its per-chunk (`prompts/chunk_review.md`) and whole-diff lens reviewers,
     so a project's rules reach every reviewer whatever the commit's size.
...
Test discipline (per top-level `~/.claude/CLAUDE.md`): every rule a
change states needs a same-diff test that pins it — one table-driven
test per rule, not a test per branch. The AI review hook blocks a rule
no test pins.
...
python ~/.claude/review/stats_cli.py --project my-project
...
1. **Coverage gate** — measured coverage of new lines/branches must reach the configured threshold, 95% by default (`diff-cover` against the compare branch).
...
To keep the gate but lower the bar, set a threshold instead. It is the
percentage of **new production lines** that must be covered; the default is
`95` — tests pin the behaviour a change states, not every line, so a defensive
branch may stay uncovered on purpose. A run below the threshold is blocked; a run above
it passes and still lists the uncovered lines, so nothing is hidden.
...
`threshold` must be a finite number between `0` and `100`. Anything else — a
string, `true`, `nan`, a negative value, or one above 100 — is a setup error
(exit 2), not a silently ignored value. `nan` in particular would compare
False against every measured percentage and quietly disable the gate.
