...
def _build_jobs(
    ...
    from hook import append_project_rules

    # repo_root, not cwd: the chunk reviewers must hear the project's rules
    # whichever directory the caller runs from.
    chunk_review_prompt = append_project_rules(_read_text(CHUNK_REVIEW_PROMPT), repo_root)
