...
Ensure symlinks exist before launching. Run once per clone or worktree. Do not commit these symlinks — they point to user-local paths. The setup is idempotent — safe to re-run.
...
If .claude/review_prompt.md exists, read it — project-specific review rules (severity overrides, design decisions to treat as intentional). Apply them during your review.
Spec file: {spec_path}
...
opencode run --pure \
  ...
  --format json \
  "Read .claude/agents-global/{agent-name}.md — follow these instructions exactly.
If .claude/review_prompt.md exists, read it — project-specific review rules. Apply them during your review.
Spec file: {spec_path}
Working directory: {worktree_path}
Base branch for diff: {base_branch}
