...
A rule the change states with no pinning test is almost always a
legitimate gap. OVERTURN only when the finding is demonstrably wrong:
...
- The cited unit is a private `_prefixed` helper with no new public
  entry point.
- The finding demands a separate test per branch, write path, field or
  cell for a rule that one table-driven test in the diff already pins.
- The finding demands a test for something that is not behavior —
  which of two valid refusals wins, private machinery, which mechanism
  refused, a framework convention, transient form state, a path only
  an API caller reaches, static message text, a race.
- The finding asks for a test only to reach a line or branch; the
  preflight's coverage threshold already decided lines.
...
grounds to overturn. The policy is behavior-first: every rule the
change states is pinned by a test; lines and branches are not the unit.
