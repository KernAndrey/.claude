...
(cited unit already covered, pure rename, stdlib-handler shape,
log-only branch, idempotent bootstrap, declarative view, cosmetic
kwarg), or when it demands a separate test per branch / path / field /
cell for a rule one table-driven test already pins, a test for
non-behavior (which of two valid refusals wins, private machinery,
which mechanism refused, a framework convention, transient form state,
an API-only path, static message text, a race), or a test only to
reach a line. The policy is behavior-first: lines and branches are not
the unit. See the single-backend arbiter prompt for full per-category
criteria.
