...
Behavior-first: flag every rule the chunk adds or changes that no test
in the diff pins — a test pins a rule, not a line or a branch. The
preflight already measured line coverage against the configured
threshold (95% by default).

- **Requires a test:** a new public function / method / class with
  business logic; a branch with a distinct observable effect; a new
  error path or refusal; a behavior-changing signature change; a new
  or changed access rule (record rule, model access row, field
  `groups=`), which is behavior even though declarative.
- **One rule is one unit:** a rule across fields, write paths, roles,
  statuses or input classes is pinned by one table-driven test
  (`subTest` / `parametrize`), one case per cell. One `[CRITICAL]` per
  unpinned rule — never one per branch, path, field or cell.
- **Exclusions (return clean):** pure config / docs; shell scripts;
  renames; test files; private `_prefixed` helpers without a new public
  entry point; declarative / metadata changes; category A
  stdlib-delegated error handlers; category B log-only branches;
  category C idempotent bootstrap wrappers; category D declarative
  view/XML conditionals; category E cosmetic kwargs.
- **Not behavior (return clean):** which of two valid refusals wins;
  private machinery; which mechanism refused; framework conventions;
  transient form state; paths only an API caller reaches; static
  message text unless the wording is the requirement; races. Never ask
  for a test only to reach a line.
- **Tests that pin non-behavior → `[WARNING]`:** a new or modified test
  asserting an item above, repeating a rule another test pins with no
  input class of its own, or spending one method per matrix cell. A
