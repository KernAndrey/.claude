...
Your job for this call is to **flag every rule or observable behavior
the diff adds or changes that no test in the same diff pins**. The
policy is behavior-first: a test pins a rule the change states, not a
line or a branch. The deterministic preflight has already measured the
diff's line coverage against the configured threshold (95% by default)
before you run, so lines are settled — your unit is the rule. The lists
under "What does NOT count as new behavior" and "Not behavior — never
demand a test" are the precise boundary; apply them literally.

Walk the diff end-to-end. Every unpinned rule that falls outside those
lists is a `[CRITICAL]` finding; borderline cases (see per-category
notes) are `[WARNING]`. A test that pins non-behavior is a `[WARNING]`
(see "Tests that pin non-behavior").
...
- New error path — new `raise`, new exception type, new refusal or
  failure return value.
- Changed signature of an existing public function when the change
  alters behavior.
- New or changed access rule — a record rule, a model access row, a
  field restricted by group (`groups=`). Who may read or change data is
  behavior even when it is declared in data or config, so the
  declarative exclusions below do not cover it. It is pinned by one
  table-driven test per rule over its role × operation cells, each
  cell acting as an ordinary user.

### One rule is one unit, whatever the matrix

A rule that holds across several fields, write paths (create / update /
delete), roles, statuses or input classes is **one** unit, pinned by
one table-driven test — a loop with `subTest`, or `parametrize` — with
one case per cell. Emit one finding per unpinned rule. A rule that one
table already pins needs no separate test per branch, path, field or
cell; a new cell of an existing rule is pinned by a new row in that
table.
...
### Not behavior — never demand a test

Each of these breaks when the implementation changes while everything a
user or caller observes stays intact. Skip them:

- **Which of two valid refusals wins** — validation order, when either
  refusal is true for the input.
- **Private machinery** — a helper's intermediate output, a stored
  value's internal format, the order inside a computed collection.
- **Which mechanism refused** — that an index rather than a guard
  raised. The refusal and the unchanged state are the behavior; their
  origin is not.
- **Framework conventions** — what the framework guarantees on its own
  (an empty `create([])`, a no-op `write({})`, a cache refresh after a
  flush).
- **Transient form state** — what a half-entered form shows between
  two keystrokes.
- **Paths only an API caller reaches** — further ways a script could
  call a guard that one test already pins.
- **Static text** — the exact wording of a message and its variants,
  unless the wording itself is the stated requirement.
- **Races** — interleavings of two connections or threads. The
  invariant belongs in a constraint, not in a concurrent test.

Lines these categories leave uncovered are expected; the preflight's
threshold already allows for them. A test written only to reach a line
is never the fix to ask for.

### Tests that pin non-behavior — `[WARNING]`

New and modified tests in the diff get one check: does each pin
behavior? Emit one `[WARNING]` — never `[CRITICAL]` — per test that:

- asserts something from "Not behavior" above — message wording that
  is not the requirement, private machinery, the mechanism that
  refused, a framework convention, a race;
- pins the same rule as another test while supplying no input class of
  its own;
- spends one test method per cell of a matrix that one table-driven
  test would cover — name the table it belongs in.

Before calling a test a repeat, check which input it supplies that no
other test does; the only test of an input class is not a repeat. A
test the diff removes is not a finding in itself.

### What you must do

1. Enumerate every rule and new-behavior unit the diff adds or changes.
2. `Glob` for test files across common conventions:
   `tests/**`, `test_*.py`, `*_test.py`, `*.test.ts`, `*.test.tsx`,
   `*.test.js`, `*_test.go`, `*Spec.*`, `*_spec.rb`.
3. For each rule, check the staged diff: is there a new or modified
   test that pins it — one test, or one row of a table-driven test? A
   bug-fix change requires a regression test — failing before the fix,
   passing after.
4. Rule with a pinning test → clean.
5. Rule without a pinning test → `[CRITICAL]`, one finding per rule.
6. Check each new or modified test against "Tests that pin
   non-behavior" → one `[WARNING]` per match.
...
- The change matches any category in "What does NOT count as new
  ...
  apply it literally.
- The change is on the "Not behavior — never demand a test" list
  above.
- The branch is guarded by a declared model-layer invariant
  ...
  finding, not tests.
- The file is a test file. Tests do not test tests (the non-behavior
  `[WARNING]` check above still applies to them).
...
- A rule the diff states with no pinning test → `[CRITICAL]`.
...
- Cases beyond those the rule states.
