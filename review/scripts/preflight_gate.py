...
# The bar a repo gets when it configures no threshold. Tests pin the behaviour
# a change states, not every line, so the default leaves room for defensive
# branches left uncovered on purpose (SDD-004). A rejected threshold also
# resolves to it, but that path is a setup error and blocks before any comparison.
...
def _threshold_range_error(value: float, source: str) -> str | None:
    """Reject a threshold that cannot act as a bar, now that it decides the verdict.

    `nan` compares False against everything, so `percent < nan` never fails and
    the gate would pass at 0% coverage — silently disabling itself. A negative
    bar does the same. Above 100 nothing can ever reach it. All three are
    config mistakes, not policies, so they fail as setup errors instead.
    """
    if not math.isfinite(value):
        return f"{source} must be a finite number, got: {value!r}"
    if not 0.0 <= value <= 100.0:
        return f"{source} must be a percentage between 0 and 100, got: {value!r}"
    return None
...
def _load_config(repo_root: Path) -> GateConfig:  # noqa: C901,PLR0912,PLR0915
    ...
    if env_threshold is not None:
        try:
            ...
            threshold_setup_error = _threshold_range_error(threshold, "CODE_REVIEW_COVERAGE_THRESHOLD")
    else:
        # Validated like the env override above: the value is now compared
        # against a measured percentage, so a string would raise TypeError
        # mid-commit instead of failing as a config error. `bool` is rejected
        # explicitly — `threshold = true` would otherwise become 1.0.
        raw_threshold = cov.get("threshold", DEFAULT_COVERAGE_THRESHOLD)
        source = "[tool.code_review.coverage_gate.coverage_check] threshold"
        if isinstance(raw_threshold, bool) or not isinstance(raw_threshold, (int, float)):
            threshold = DEFAULT_COVERAGE_THRESHOLD
            threshold_setup_error = f"{source} must be a number, got: {raw_threshold!r}"
        else:
            threshold = float(raw_threshold)
            threshold_setup_error = _threshold_range_error(threshold, source)
    if threshold_setup_error is not None:
        threshold = DEFAULT_COVERAGE_THRESHOLD
...
class CoverageResult:
    ...
    # Measured coverage of new production lines, straight from diff-cover's
    # own JSON (see `_extract_coverage_totals`). `None` means diff-cover did
    # not tell us — the verdict then falls back to the historical strict rule
    # rather than guessing, see `_coverage_failed`.
    percent_covered: float | None = None
...
def _as_number(value: object) -> float | None:
    """A numeric JSON scalar as float; None for anything else.

    `bool` is excluded on purpose — `isinstance(True, int)` is True in Python,
    and a `true` in the report would otherwise be read as 1%.
    """
...
def _extract_coverage_totals(data: object) -> tuple[float | None, int | None, int | None]:
    """`(percent, total_lines, covered_lines)` from diff-cover's own JSON.

    diff-cover 10.5.1 emits `total_percent_covered` next to `total_num_lines`
    and `total_num_violations` (report_generator.py:221-232). Prefer its own
    figure; fall back to the counts if only those are present; report `None`
    when neither is, so the caller keeps the strict rule instead of guessing.
    """
    if not isinstance(data, dict):
        return None, None, None

    total = _as_number(data.get("total_num_lines"))
    violations = _as_number(data.get("total_num_violations"))
    have_counts = total is not None and violations is not None
    covered = int(total - violations) if have_counts else None

    percent = _as_number(data.get("total_percent_covered"))
    if percent is None and have_counts:
        # diff-cover truncates its own percentage (report_generator.py:174-176);
        # floor the derived one too, or the two routes disagree at the boundary.
        percent = 100.0 if total == 0 else float(int(covered * 100 // total))

    return percent, (int(total) if total is not None else None), covered


def _coverage_failed(result: CoverageResult | None, config: GateConfig) -> bool:
    """True when measured coverage of new production lines is below threshold.

    An unknown percentage is never read as a pass. It falls back to the
    historical rule — any uncovered line fails — which is always at least as
    strict as a threshold comparison, so a diff-cover that stops emitting the
    figure tightens this gate instead of silently opening it.

    The single definition is deliberate: the verdict in `run_gate` and the
    wording in `_format_report` must not be able to disagree.
    """
    if result is None:
        return False
    if result.percent_covered is None:
        return bool(result.findings)
    return result.percent_covered < config.coverage_threshold


def _measured_phrase(result: CoverageResult, config: GateConfig) -> str:
    """`98% of 954 new line(s) covered (threshold 80%)` — the figure and the bar.

    Never the percentage alone: 100% over zero new lines says nothing.
    """
    threshold = f"{config.coverage_threshold:g}"
    if result.percent_covered is None:
        return f"measured coverage unavailable, failing on any uncovered line (threshold {threshold}%)"
    scope = f" of {result.total_lines} new line(s)" if result.total_lines is not None else ""
    return f"{result.percent_covered:g}%{scope} covered (threshold {threshold}%)"
...
def _coverage_section(
    ...
    config: GateConfig,
    cov_ok: bool,
    c: Callable[[Callable[[str], str], str], str],
) -> list[str]:
    """The `[1/2]` block. Uncovered lines are listed whether or not they block
    the commit, so the header has to say which of the two happened."""
    count = len(result.findings)
    if not count:
        return [c(_green, "[1/2] Coverage gate — OK")]

    phrase = _measured_phrase(result, config)
    if cov_ok:
        head = c(_green, f"[1/2] Coverage gate — OK, {phrase}; {count} uncovered line(s)/branch(es) listed below")
    else:
        head = c(_red, f"[1/2] Coverage gate — {count} uncovered line(s)/branch(es), {phrase}")

    lines = [head]
    lines.extend(f"  {f.path}:{f.line} ({f.kind})" for f in result.findings[: config.max_report_lines_in_output])
    if count > config.max_report_lines_in_output:
        lines.append(f"  ... and {count - config.max_report_lines_in_output} more")
    return lines
...
def _format_report(
    ...
    lines: list[str] = []
    cov_ok = not _coverage_failed(coverage_result, config)
    ...
    if coverage_result is not None:
        lines.extend(_coverage_section(coverage_result, config, cov_ok, c))
...
def _run_coverage_check(config: GateConfig, repo_root: Path) -> CoverageResult:  # noqa: C901,PLR0911,PLR0912,PLR0915
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        percent, total_lines, covered_lines = _extract_coverage_totals(data)
        return CoverageResult(
            findings=findings,
...
def run_gate(repo_root: Path | None = None) -> int:  # noqa: C901,PLR0911,PLR0912,PLR0915
    ...
    # Coverage blocks the commit only when measured coverage is below the
    # configured threshold; the assert gate has no threshold and blocks on any
    # finding. `--fail-under` still goes to diff-cover, but its exit code is
    # not the verdict: rc 1 also means "git not found", before any report.
    coverage_blocks = _coverage_failed(coverage_result, config)
    assert_blocks = bool(assert_result is not None and assert_result.findings)

    if coverage_blocks or assert_blocks:
        print(_format_report(coverage_result, assert_result, config), file=sys.stderr)
        return 1

    if coverage_result is not None and coverage_result.findings:
        # Passing on threshold, but the uncovered lines are still worth seeing.
        print(_format_report(coverage_result, assert_result, config), file=sys.stderr)
