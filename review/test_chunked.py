...
# ---------------------------------------------------------------------------
# Project review rules reach chunk and whole-diff reviewers (SDD-004)
# ---------------------------------------------------------------------------
...
@pytest.mark.parametrize(
    ("job_kind", "base"),
    [("chunk", "CHUNK PROMPT"), ("wholediff", "COMMON\n\n---\n\nBUGS LENS")],
)
@pytest.mark.parametrize("rules_present", [True, False], ids=["rules", "no-rules"])
def test_chunked_reviewer_system_prompt_carries_project_rules(
    tmp_path: Path,
    ...
    """Both reviewer layers of the chunked path hear the project's rules.

    The chunk layer resolves them against ``repo_root``, so its row runs from a
    different cwd; the whole-diff lens layer reads them from the cwd, which is
    the repo root when git runs the hook.
    """
    prompts = tmp_path / "prompts"
    prompts.mkdir()
    (prompts / "chunk_review.md").write_text("CHUNK PROMPT", encoding="utf-8")
    (prompts / "common.md").write_text("COMMON", encoding="utf-8")
    (prompts / "bugs.md").write_text("BUGS LENS", encoding="utf-8")
    repo = tmp_path / "repo"
    (repo / ".claude").mkdir(parents=True)
    if rules_present:
        (repo / ".claude" / "review_prompt.md").write_text(_PROJECT_RULE + "\n", encoding="utf-8")
    elsewhere = tmp_path / "elsewhere"
    elsewhere.mkdir()
    monkeypatch.chdir(elsewhere if job_kind == "chunk" else repo)

    inputs = chunked._RunInputs(manifest_text="dummy")
    inputs.manifest = {"chunks": [{"id": "models", "files": [{"path": "a.py"}]}], "default_related_files": []}
    captured: dict[str, str] = {}

    def fake_reviewer(
        backend_cfg: RunnerConfig, system_prompt: str, user_prompt: str, job_id: str
    ) -> tuple[str, str, int, str, str]:
        captured[job_id.split(":")[0]] = system_prompt
        return "", "", 0, "ok", backend_cfg.backend

    with (
        patch("chunked.CHUNKED_BACKENDS", [RunnerConfig("claude", "sonnet")]),
        patch("chunked.config.ALLOWED_LENSES", frozenset({"bugs"})),
        patch("chunked.CHUNK_REVIEW_PROMPT", prompts / "chunk_review.md"),
        patch("hook.LENS_DIR", prompts),
        patch("chunked._run_one_reviewer", side_effect=fake_reviewer),
    ):
        for job in chunked._build_jobs(inputs, _diff_for_new_file("a.py", ["x"]), "a.py", False, repo):
            job()

    assert captured[job_kind] == base + (_RULES_SECTION if rules_present else "")
