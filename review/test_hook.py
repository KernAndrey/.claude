...
def _consolidation_with_no_clusters() -> MagicMock:
    """A consolidation result carrying no findings — the OK path."""
    return MagicMock(upheld_clusters=[], clusters=[])


def _ok_backend_result(review_text: str) -> MagicMock:
    return MagicMock(
        status="ok",
        review_text=review_text,
        fallback_used=False,
        cfg=MagicMock(backend="codex", model="gpt-5.6-terra"),
    )


def test_consolidation_display_warning_lines_stay_parseable() -> None:
    """The rendered display is re-parsed by `extract_warning_lines` on the
    OK path, so the lines it writes must still look like findings. Adding a
    bullet to a line that already carries one produced `- - [WARNING]`,
    which the anchored regex does not match."""
    from hook import _render_consolidation_display, extract_warning_lines

    warning = "- [WARNING] foo.py:12 — `helper()` — duplicated in bar.py."
    display = _render_consolidation_display(
        [_ok_backend_result(f"### Section 2 — Findings\n{warning}\nSummary: 0 CRITICAL, 1 WARNING across 1 files.")],
        _consolidation_with_no_clusters(),
    )

    assert extract_warning_lines(display) == [warning]
    assert "- - [WARNING]" not in display


def test_consolidation_display_keeps_id_prefixed_warning_parseable() -> None:
    """Arbiter-tagged findings arrive as `[F1] [WARNING] …` with their own
    bullet — the same double-bullet hazard, and the shape multi-backend
    runs actually emit."""
    from hook import _render_consolidation_display, extract_warning_lines

    warning = "- [codex-F1] [WARNING] bar.py:7 — stale comment."
    display = _render_consolidation_display(
        [_ok_backend_result(f"{warning}\nSummary: 0 CRITICAL, 1 WARNING across 1 files.")],
        _consolidation_with_no_clusters(),
    )
...
def test_ok_verdict_surfaces_warnings_to_stderr() -> None:
    """End to end on the OK path: a review with warnings must reach the
    developer's terminal. The banner gate at the end of the pipeline
    re-parses the rendered display, so a display that mangles its own
    warning lines leaves the commit looking clean."""
    import hook
    ...
    warned: list[str] = []
    with (
        patch("orchestrator.run_multi_backend", return_value=[_ok_backend_result(review)]),
        patch("consolidation.consolidate", return_value=_consolidation_with_no_clusters()),
        patch("hook._persist_run"),
        patch("hook.warn", side_effect=warned.append),
        patch("hook.info"),
    ):
        verdict = hook._run_multi_backend_pipeline("diff", "foo.py", False)

    assert verdict == "OK"
    assert any("Review notes (non-blocking warnings)" in w for w in warned)
    assert any("duplicated in bar.py" in w for w in warned)
...
# ---------------------------------------------------------------------------
# Project review rules reach every reviewer system prompt (SDD-004)
# ---------------------------------------------------------------------------

_PROJECT_RULE = "Project rule: a view change needs no test."
_RULES_SECTION = f"\n\n---\n\n## Project-specific review rules:\n{_PROJECT_RULE}"


def _repo_with_rules(root: Path, rules: str | None) -> Path:
    """A repo dir whose `.claude/review_prompt.md` holds ``rules`` (absent when None)."""
    (root / ".claude").mkdir(parents=True)
    if rules is not None:
        (root / ".claude" / "review_prompt.md").write_text(rules + "\n", encoding="utf-8")
    return root
...
@pytest.mark.parametrize(
    ("builder", "base"),
    [
        (build_system_prompt, "GLOBAL"),
        (lambda: build_lens_system_prompt("tests"), "COMMON\n\n---\n\nTESTS LENS"),
    ],
    ids=["single-call", "fan-out-lens"],
)
@pytest.mark.parametrize("rules_present", [True, False], ids=["rules", "no-rules"])
def test_reviewer_system_prompt_carries_project_rules(
    tmp_path: Path,
    ...
    """The project's rules reach the single-call and the fan-out lens reviewer
    alike, so a large commit hears them as well as a small one."""
    prompts = tmp_path / "prompts"
    prompts.mkdir()
    (prompts / "combined.md").write_text("GLOBAL", encoding="utf-8")
    (prompts / "common.md").write_text("COMMON", encoding="utf-8")
    (prompts / "tests.md").write_text("TESTS LENS", encoding="utf-8")
    monkeypatch.chdir(_repo_with_rules(tmp_path / "repo", _PROJECT_RULE if rules_present else None))

    with patch("hook.GLOBAL_PROMPT", prompts / "combined.md"), patch("hook.LENS_DIR", prompts):
        prompt = builder()

    assert prompt == base + (_RULES_SECTION if rules_present else "")


def test_append_project_rules_keeps_an_empty_prompt_empty(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    """"" means the global prompt is missing; the rules alone must not become a prompt."""
    monkeypatch.chdir(_repo_with_rules(tmp_path / "repo", _PROJECT_RULE))
    assert append_project_rules("") == ""


def test_append_project_rules_reads_the_given_repo_root_not_cwd(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    repo = _repo_with_rules(tmp_path / "repo", _PROJECT_RULE)
    monkeypatch.chdir(_repo_with_rules(tmp_path / "elsewhere", "Rule of the wrong directory."))
    assert append_project_rules("BASE", repo) == "BASE" + _RULES_SECTION
