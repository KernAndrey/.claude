...
def test_load_config_defaults_when_missing_pyproject(tmp_path: Path) -> None:
    ...
    assert cfg.coverage_threshold == 95.0
...
# ---------------------------------------------------------------------------
# Threshold gate — the configured threshold decides pass/fail
#
# Before this fix the threshold was passed to diff-cover as `--fail-under` and
# the answer thrown away: ANY uncovered new production line failed the gate,
# whatever the threshold said. The verdict now compares diff-cover's measured
# `total_percent_covered` against `config.coverage_threshold`.
# ---------------------------------------------------------------------------


def _cov_result(pct: float | None, n_uncovered: int = 1, total_lines: int | None = None) -> CoverageResult:
    """A CoverageResult carrying `n_uncovered` findings at `pct` measured coverage."""
    findings = [CoverageFinding("a.py", i, "line") for i in range(1, n_uncovered + 1)]
    return CoverageResult(findings=findings, percent_covered=pct, total_lines=total_lines)


def _gate_rc(cov: CoverageResult, config: GateConfig) -> int:
    """run_gate() with the coverage result and config injected, assert gate clean."""
    with (
        patch("scripts.preflight_gate._load_config", return_value=config),
        patch("scripts.preflight_gate._run_coverage_check", return_value=cov),
        patch("scripts.preflight_gate._diff_has_production_python", return_value=True),
        patch("scripts.preflight_gate._run_assert_check", return_value=AssertResult()),
        patch("scripts.preflight_gate._run_assert_check_js", return_value=AssertResult()),
...
def test_run_gate_default_threshold_fails_below_ninety_five() -> None:
    """A project that configured NO threshold gets the default bar of 95.
    94% is below it → fail.

    This is the row that catches a `<=` slip or a threshold defaulting lower —
    every threshold-80 test below would still pass with such a bug.
    """
    assert _gate_rc(_cov_result(94.0, total_lines=100), GateConfig()) == 1


def test_run_gate_default_threshold_passes_at_ninety_five() -> None:
    """The default's boundary: 95% with lines still uncovered passes.

    Tests pin behaviour, not every line (SDD-004), so the default no longer
    demands 100 — under the old default this run failed.
    """
    assert _gate_rc(_cov_result(95.0, n_uncovered=5, total_lines=100), GateConfig()) == 0


def test_run_gate_below_configured_threshold_fails() -> None:
    cfg = GateConfig(coverage_threshold=80.0)
    assert _gate_rc(_cov_result(70.0, n_uncovered=3, total_lines=10), cfg) == 1


def test_run_gate_above_configured_threshold_passes() -> None:
    """The whole point of the fix: 98% at threshold 80 must pass."""
    cfg = GateConfig(coverage_threshold=80.0)
    assert _gate_rc(_cov_result(98.0, n_uncovered=18, total_lines=954), cfg) == 0


def test_run_gate_exactly_at_threshold_passes() -> None:
    """Boundary: the comparison is `<`, not `<=`."""
    cfg = GateConfig(coverage_threshold=80.0)
    assert _gate_rc(_cov_result(80.0, total_lines=10), cfg) == 0


def test_run_gate_passing_threshold_still_lists_uncovered_lines(
    capsys: pytest.CaptureFixture[str],
) -> None:
    """A passing run stays informative — the uncovered lines are still printed."""
    cfg = GateConfig(coverage_threshold=80.0, output_format="plain")
    assert _gate_rc(_cov_result(98.0, n_uncovered=2, total_lines=954), cfg) == 0
    err = capsys.readouterr().err
    assert "a.py:1 (line)" in err
    assert "a.py:2 (line)" in err


def test_run_gate_passing_threshold_omits_bypass_hint(capsys: pytest.CaptureFixture[str]) -> None:
    """The bypass hint belongs to a blocked commit only."""
    cfg = GateConfig(coverage_threshold=80.0, output_format="plain")
    _gate_rc(_cov_result(98.0, total_lines=954), cfg)
    assert "Bypass:" not in capsys.readouterr().err


def test_run_gate_failing_threshold_shows_bypass_hint(capsys: pytest.CaptureFixture[str]) -> None:
    cfg = GateConfig(coverage_threshold=80.0, output_format="plain")
    _gate_rc(_cov_result(70.0, total_lines=10), cfg)
    assert "Bypass:" in capsys.readouterr().err


def test_run_gate_no_new_lines_passes() -> None:
    """Nothing in the diff — diff-cover reports 100% over 0 lines. Must pass."""
    cov = CoverageResult(findings=[], percent_covered=100.0, total_lines=0, covered_lines=0)
    assert _gate_rc(cov, GateConfig(coverage_threshold=80.0)) == 0


def test_run_gate_unknown_percentage_with_findings_fails() -> None:
    """Unknown coverage must never read as a pass: it falls back to the
    historical strict rule, which is never weaker than any threshold."""
    assert _gate_rc(_cov_result(None), GateConfig(coverage_threshold=0.0)) == 1


def test_run_gate_unknown_percentage_without_findings_passes() -> None:
    """No violations means every measured new line is covered — passing is correct."""
    assert _gate_rc(CoverageResult(findings=[]), GateConfig(coverage_threshold=100.0)) == 0


# ---------------------------------------------------------------------------
# Threshold gate — report wording tells the two states apart
# ---------------------------------------------------------------------------


def test_format_report_below_threshold_names_both_numbers() -> None:
    cov = _cov_result(70.0, n_uncovered=3, total_lines=10)
    report = _format_report(cov, None, GateConfig(output_format="plain", coverage_threshold=80.0))
    assert "[1/2] Coverage gate — 3 uncovered line(s)/branch(es), 70% of 10 new line(s) covered (threshold 80%)" in report


def test_format_report_above_threshold_reads_as_ok() -> None:
    cov = _cov_result(98.0, n_uncovered=18, total_lines=954)
    report = _format_report(cov, None, GateConfig(output_format="plain", coverage_threshold=80.0))
    assert (
        "[1/2] Coverage gate — OK, 98% of 954 new line(s) covered (threshold 80%); "
        "18 uncovered line(s)/branch(es) listed below"
    ) in report


def test_format_report_pass_and_fail_headers_differ() -> None:
    """Same findings, same percentage, different threshold — a reader must be
    able to tell a passing run from a failing one."""
    cov = _cov_result(98.0, n_uncovered=18, total_lines=954)
    passing = _format_report(cov, None, GateConfig(output_format="plain", coverage_threshold=80.0))
    failing = _format_report(cov, None, GateConfig(output_format="plain", coverage_threshold=100.0))
    assert passing.splitlines()[0] != failing.splitlines()[0]
    assert passing.splitlines()[0].startswith("[1/2] Coverage gate — OK")
    assert failing.splitlines()[0].startswith("[1/2] Coverage gate — 18 uncovered")


def test_format_report_unknown_percentage_says_so() -> None:
    """A renamed diff-cover key degrades to the strict rule — say it out loud
    rather than silently tightening the gate."""
    cov = _cov_result(None, n_uncovered=2)
    report = _format_report(cov, None, GateConfig(output_format="plain", coverage_threshold=80.0))
    assert "2 uncovered line(s)/branch(es)" in report
    assert "measured coverage unavailable" in report


def test_format_report_percentage_without_totals_omits_denominator() -> None:
    cov = CoverageResult(findings=[CoverageFinding("a.py", 1, "line")], percent_covered=70.0)
    report = _format_report(cov, None, GateConfig(output_format="plain", coverage_threshold=80.0))
    assert "70% covered (threshold 80%)" in report
    assert "new line(s)" not in report


def test_format_report_fractional_percentage_not_rounded_to_int() -> None:
    cov = _cov_result(79.5, total_lines=200)
    report = _format_report(cov, None, GateConfig(output_format="plain", coverage_threshold=80.0))
    assert "79.5% of 200 new line(s) covered (threshold 80%)" in report


# ---------------------------------------------------------------------------
# Threshold gate — the percentage comes from diff-cover's own JSON
# ---------------------------------------------------------------------------


def _run_coverage_check_with_json(tmp_path: Path, json_out: dict[str, Any], rc: int = 1) -> CoverageResult:
    """Drive _run_coverage_check against a faked diff-cover emitting `json_out`."""
    repo = _make_repo(tmp_path)
    (repo / "coverage.xml").write_text('<coverage branch-rate="1.0"></coverage>')
    td = tmp_path / "cov_td"
    td.mkdir()
    ...
    with (
        ...
        return _run_coverage_check(GateConfig(), repo)


def test_coverage_check_reads_total_percent_covered_from_json(tmp_path: Path) -> None:
    """`total_percent_covered` is the key diff-cover 10.5.1 emits
    (report_generator.py:221-232) — take its figure, don't recompute one."""
    result = _run_coverage_check_with_json(
        tmp_path,
        {
            "src_stats": {"a.py": {"violation_lines": [7]}},
            "total_num_lines": 954,
            "total_num_violations": 18,
            "total_percent_covered": 98,
        },
    )
    assert result.percent_covered == 98.0
    assert result.total_lines == 954
    assert result.covered_lines == 936
    assert len(result.findings) == 1


def test_coverage_check_derives_percentage_from_counts_when_key_absent(tmp_path: Path) -> None:
    """Older/renamed schema: derive from the counts diff-cover does emit."""
    result = _run_coverage_check_with_json(
        tmp_path,
        {"src_stats": {"a.py": {"violation_lines": [7]}}, "total_num_lines": 10, "total_num_violations": 2},
    )
    assert result.percent_covered == 80.0
    assert result.total_lines == 10
    assert result.covered_lines == 8


def test_coverage_check_derived_percentage_is_floored_like_diff_cover(tmp_path: Path) -> None:
    """diff-cover truncates its own percentage (report_generator.py:174-176);
    the derived route must floor too or the two routes disagree."""
    result = _run_coverage_check_with_json(
        tmp_path,
        {"src_stats": {"a.py": {"violation_lines": [7]}}, "total_num_lines": 954, "total_num_violations": 1},
    )
    assert result.percent_covered == 99.0


def test_coverage_check_zero_new_lines_is_hundred_percent(tmp_path: Path) -> None:
    """No new lines → no division by zero, and 100% like diff-cover itself."""
    result = _run_coverage_check_with_json(
        tmp_path,
...
def test_coverage_check_no_totals_leaves_percentage_unknown(tmp_path: Path) -> None:
    """Neither key present → None, so the caller keeps the strict rule."""
    result = _run_coverage_check_with_json(tmp_path, {"src_stats": {"a.py": {"violation_lines": [7]}}})
    assert result.percent_covered is None
    assert result.total_lines is None
    assert len(result.findings) == 1


def test_coverage_check_rejects_boolean_percentage(tmp_path: Path) -> None:
    """`isinstance(True, int)` is True in Python — a bool must not be read as 1%."""
    result = _run_coverage_check_with_json(
        tmp_path,
        {
            "src_stats": {"a.py": {"violation_lines": [7]}},
            ...
            "total_percent_covered": True,
        },
    )
    assert result.percent_covered == 80.0


def test_coverage_check_counts_without_violations_key_stay_unknown(tmp_path: Path) -> None:
    """A denominator alone cannot give a percentage — guessing 0 violations
    would report 100% and open the gate."""
    result = _run_coverage_check_with_json(
        tmp_path,
        {"src_stats": {"a.py": {"violation_lines": [7]}}, "total_num_lines": 10},
    )
    assert result.percent_covered is None


# ---------------------------------------------------------------------------
# Threshold gate — a malformed TOML threshold fails loudly
#
# The env path was already validated; the TOML path was not, and an
# unvalidated value now reaches a numeric comparison.
# ---------------------------------------------------------------------------


def test_load_config_non_numeric_toml_threshold(tmp_path: Path) -> None:
    _write_pyproject(
        tmp_path,
        '[tool.code_review.coverage_gate.coverage_check]\nthreshold = "high"\n',
    )
...
def test_load_config_boolean_toml_threshold_rejected(tmp_path: Path) -> None:
    """`threshold = true` would otherwise become 1.0 — a gate that passes at 1%."""
    _write_pyproject(
        tmp_path,
        "[tool.code_review.coverage_gate.coverage_check]\nthreshold = true\n",
    )
...
def test_load_config_numeric_toml_threshold_accepted(tmp_path: Path) -> None:
    _write_pyproject(tmp_path, "[tool.code_review.coverage_gate.coverage_check]\nthreshold = 80\n")
    cfg = _load_config(tmp_path)
    assert cfg.coverage_threshold == 80.0
    assert cfg.threshold_setup_error is None


def test_run_gate_non_numeric_toml_threshold_returns_two(tmp_path: Path) -> None:
    """A bad threshold stops the gate before any check runs — rc 2, not a
    lenient comparison and not a crash."""
    _write_pyproject(
        tmp_path,
        '[tool.code_review.coverage_gate.coverage_check]\nthreshold = "high"\n',
    )
    with patch("scripts.preflight_gate._run_coverage_check") as m_cov:
        assert run_gate(tmp_path) == 2
    m_cov.assert_not_called()


# ---------------------------------------------------------------------------
# Threshold gate — a threshold that cannot act as a bar is a setup error
#
# `nan` compares False against everything, so `percent < nan` never fails and
# the gate passes at 0% coverage. A negative bar is unreachable from below and
# has the same effect. Both parse fine as TOML/float, so the type check alone
# is not enough now that the value actually decides the verdict.
# ---------------------------------------------------------------------------


@pytest.mark.parametrize("literal", ["nan", "-5", "-0.1", "101", "inf"])
def test_load_config_toml_threshold_out_of_range_is_setup_error(tmp_path: Path, literal: str) -> None:
    _write_pyproject(tmp_path, f"[tool.code_review.coverage_gate.coverage_check]\nthreshold = {literal}\n")
    cfg = _load_config(tmp_path)
    assert cfg.threshold_setup_error is not None
    assert cfg.coverage_threshold == 95.0


@pytest.mark.parametrize("value", ["nan", "-5", "-0.1", "101", "inf"])
def test_load_config_env_threshold_out_of_range_is_setup_error(tmp_path: Path, value: str) -> None:
    os.environ["CODE_REVIEW_COVERAGE_THRESHOLD"] = value
    try:
        cfg = _load_config(tmp_path)
        assert cfg.threshold_setup_error is not None
        assert cfg.coverage_threshold == 95.0
...
def test_load_config_nan_threshold_names_finiteness(tmp_path: Path) -> None:
    """The two rejections carry different messages — a `nan` is not an
    out-of-range percentage, and the report should not say it is."""
    _write_pyproject(tmp_path, "[tool.code_review.coverage_gate.coverage_check]\nthreshold = nan\n")
    cfg = _load_config(tmp_path)
    assert cfg.threshold_setup_error is not None
    assert "finite" in cfg.threshold_setup_error


def test_load_config_negative_threshold_names_the_range(tmp_path: Path) -> None:
    _write_pyproject(tmp_path, "[tool.code_review.coverage_gate.coverage_check]\nthreshold = -5\n")
    cfg = _load_config(tmp_path)
    assert cfg.threshold_setup_error is not None
    assert "between 0 and 100" in cfg.threshold_setup_error


@pytest.mark.parametrize("literal", ["0", "0.0", "100", "100.0", "79.5"])
def test_load_config_toml_threshold_in_range_accepted(tmp_path: Path, literal: str) -> None:
    """0 stays valid — "require nothing" is a real, if unusual, policy."""
    _write_pyproject(tmp_path, f"[tool.code_review.coverage_gate.coverage_check]\nthreshold = {literal}\n")
    cfg = _load_config(tmp_path)
    assert cfg.threshold_setup_error is None
    assert cfg.coverage_threshold == float(literal)


def test_load_config_env_threshold_zero_accepted(tmp_path: Path) -> None:
    os.environ["CODE_REVIEW_COVERAGE_THRESHOLD"] = "0"
    try:
        cfg = _load_config(tmp_path)
        assert cfg.threshold_setup_error is None
...
def test_run_gate_nan_threshold_blocks_instead_of_passing_everything(tmp_path: Path) -> None:
    """The regression that matters: before the range check, `nan` let a commit
    through with 0% coverage of new lines."""
    _write_pyproject(tmp_path, "[tool.code_review.coverage_gate.coverage_check]\nthreshold = nan\n")
    with patch("scripts.preflight_gate._run_coverage_check") as m_cov:
        assert run_gate(tmp_path) == 2
    m_cov.assert_not_called()
...
def test_run_gate_zero_threshold_passes_with_uncovered_lines() -> None:
    """threshold 0 means "require nothing" — it must pass, not error."""
    assert _gate_rc(_cov_result(0.0, n_uncovered=5, total_lines=10), GateConfig(coverage_threshold=0.0)) == 0
