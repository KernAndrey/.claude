---
name: kimi
description: Delegate work to the Kimi CLI agent (Moonshot AI's `kimi` on Kimi K3, 1M context, runs locally with PreToolUse safety hooks and rule injection). Kimi quota is scarce — use this skill only on an explicit request: the user says "use kimi", "delegate to kimi", "let kimi do it", "ask kimi to ...", "send to kimi", or runs `/kimi`. Then it covers: implementation/refactor of a well-scoped change; read-only security/perf/anti-pattern/dead-code audit; research and explanation of how some part of the codebase works (with `path:line` citations); peer review / second opinion on a ready diff; deterministic transform tasks (translate comments, generate docs from code, reverse-spec, generate tests). Supports parallel fan-out for independent multi-angle work. Without an explicit request, use Claude's own agents instead. Runs `kimi -p` (kimi-code ≥0.19) in the background with rule injection from `~/.kimi/lib/build_context.py` so kimi follows the same `CLAUDE.md` and `.claude/rules/*` you do; waits for the `=== KIMI_DONE` marker; returns kimi's output verbatim.
---
