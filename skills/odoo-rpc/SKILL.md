...
description: >
  ...
  wants to read Odoo data outside an Odoo MCP connector, inspect a model's
...
db    = "pri_seed"
login = "admin"
api_key = "..."          # or password = "...", or api_key_env = "ACME_ODOO_KEY"
...
## Reading through an MCP connector instead

When a database already has a read-only Odoo MCP connector configured, it is
available (`list_models`, `get_schema`, `search_read`, `read`,
`read_group`, `search_count`) and needs no credentials or tunnel URL. Prefer it
it for reads there, and use this skill when you need another instance or a write.
