...
2. `.claude/rules/readable-syntax.md`, when one exists — which constructs this
   project considers unreadable, and the plainer form it asks for instead.
3. The language rule file, when one exists — `.claude/rules/odoo19-python.md`
   and its siblings — for naming conventions the linter cannot express.
4. `.claude/rules/client-ticket-is-the-spec.md`, when one exists, plus the
   ticket itself: names are judged against the ticket's vocabulary, not only
   against the prefix rules.
5. The linter config, when one exists — `ruff.toml`, `pyproject.toml`,
   `.pylintrc`, `.pre-commit-config.yaml`.
6. Three sibling files in the same module as the diff — the convention as it is
   actually practised, including comment density, docstring phrasing and how
   private methods are named.

<critical>
Projects here hold opposite policies. Some repos keep ticket numbers,
...
Other repos use all three freely in their own code. Judge the diff by the file
you read in step 1 — never by a convention you remember from another project.
</critical>

## Scope: the diff, never the repository

Review only the lines the diff adds or changes. The posture in those repos is explicit:
...
1. Read `.claude/rules/code-comments.md` end to end, and
   `.claude/rules/readable-syntax.md` when the project has one. Build a
   checklist from both — one line per rule they state. That checklist is your
   work queue. The second file judges how a line is written, the first judges
   what is written around it; a diff can conform to one and not the other.
2. Read the linter config. Rules it already enforces belong to the pre-commit
   ...
   config file exists** — a repo can carry no linter
   config at all, and then nothing is carved out there.
...
- syntax a reader must decode rather than read: a condition stated as its
  complement, one call that checks and inserts and returns at once, generality
  no caller uses, a comprehension carrying two loops
- method naming: the prefix the language rule file prescribes (`_compute_`,
  `_search_`, `_inverse_`, `_get_` for a helper returning records), and whether
  the noun in the name is the ticket's word or one the author coined. A name the
  ticket does not use, where the ticket has its own word for the same thing, is
  a finding against `client-ticket-is-the-spec.md`
...
```
DEPTH:
- Rule files read: {path, path}, {N} rules extracted from each
  (name `readable-syntax.md` explicitly, or say the project has none)
