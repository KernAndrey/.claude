...
<!-- For every non-trivial Behavior rule, one concrete input→output example
     ...
     ### Example: Archive a lead-stage contact by confirming their order
     ...
     After:
       ...
       partner.setup_info_ids += [{event_type: 'contact_first_order',
                                   order_id: 5, order_ref: 'SO-1042'}]
-->
...
<!-- The test plan, AC by AC. Owned by Analyst and written in business/behavior
     ...
     unit | integration | e2e. A test pins a rule the spec states, not a
     line or a branch. Each entry lists:
       - Rule: the rule the AC states — literal input, literal observable
         that proves it holds
       - Refusal: every refusal the AC or Behavior states — rejected input,
         missing permission, absent record, conflicting state — with the
         state it leaves unchanged. One line each. When the spec states
         none, write `no refusal stated` so the reader sees it was
         considered.
       - Cases: the values or input classes Behavior distinguishes for this
         rule — each enumeration value, state transition, or role ×
         operation cell of an access rule. They become rows of one
         table-driven test, not one test each; `n/a` when the rule has a
         single case. Variants Behavior does not distinguish are not listed.
       - Not tested: what this AC touches that is not behavior, with its
         category — a race between connections or threads, which of two
         valid refusals wins, which mechanism refused, message wording that
         is not the requirement. Omit the line when nothing applies.
     ...
     Close with two coverage lines that map the rest of the spec into the plan:
       - Edge Cases covered: every row of ## Edge Cases & Risks → the entry
         that tests it, or `not tested — <category>`
     ...
     - Rule: employee `active=True`, reason "Retired" → `active=False`,
       `archive_reason='Retired'`, one audit row with `archived_by=uid`.
     - Refusal: employee already archived → UserError, no second audit row.
     - Refusal: user outside `hr.group_hr_manager` → AccessError, record
       unchanged.
     - Cases: reasons "Retired", "Resigned", "Other" with free text → one
       table; "Other" with empty free text → a ValidationError row.
     - Not tested: two managers archiving the same employee at once — race.
     ...
     - Rule: input containing a `<script>` block → output identical minus
       that block.
     - Refusal: no refusal stated — pure function, total over string input.
     - Cases: `<script>`, `<iframe>`, inline `onclick=` → one table.
     - Fixtures: tests/fixtures/signatures/*.html (3 real samples).
     - Idempotency: sanitize(sanitize(x)) == sanitize(x).
     - Mocks: none.

     Edge Cases covered: risk 1 → AC-1 refusal 2; risk 2 → not tested — race.
     Examples covered: "Archive a lead-stage contact" → AC-1 rule.
